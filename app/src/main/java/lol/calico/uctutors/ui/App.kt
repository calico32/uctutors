...
import lol.calico.uctutors.ui.page.ExplorePage
...
fun App() {
  ...
  val currentPageDestination = pageController
    .currentBackStackEntryAsState().value?.destination

  LaunchedEffect(Unit) {
    tokenStorage.propagateToken(scope)
  }
  ...
  UCTutorsTheme {
    ...
    CompositionLocalProvider(
      ...
      NavHost(appController, startDestination = "loading") {
        ...
        composable("register") {
          RegisterPage(onRegister = accountHandler::register)
        }
        composable("pages") {
          AppLayout(
            title = currentPageDestination?.route?.replaceFirstChar(Char::uppercaseChar)
              ?: "UCTutors",
          ) { contentPadding ->
            NavHost(pageController, startDestination = "home") {
              composable("home") {
                HomePage(contentPadding, onSignOut = accountHandler::signOut)
              }
              composable("calendar") {
                CalendarPage(contentPadding)
              }
              composable("explore") {
                ExplorePage(contentPadding)
              }
              composable("you") {
                YouPage(contentPadding)
              }
            }
          }
        }
